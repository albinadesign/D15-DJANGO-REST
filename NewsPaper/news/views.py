...
class PostViewset(viewsets.ModelViewSet):
    ...
    filterset_fields = ['dateCreation', 'postCategory', 'title']
    ...
    def get_queryset(self):
        queryset = Post.objects.all()
        dateCreation = self.request.query_params.get('dateCreation', None)
        postCategory = self.request.query_params.get('postCategory', None)
        title = self.request.query_params.get('title', None)
        if dateCreation is not None:
            queryset = queryset.filter(post__dateCreation=dateCreation)
        if postCategory is not None:
            queryset = queryset.filter(post__postCategory=postCategory)
...
class NewsSearchList(ListView, View):
